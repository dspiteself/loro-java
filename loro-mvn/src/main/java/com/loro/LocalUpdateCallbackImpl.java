...
public class LocalUpdateCallbackImpl implements AutoCloseable, LocalUpdateCallback {
  ...
  public LocalUpdateCallbackImpl(Pointer pointer) {
   ...
   */
  public LocalUpdateCallbackImpl(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("LocalUpdateCallbackImpl object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("LocalUpdateCallbackImpl call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(status -> {
          UniffiLib.INSTANCE.uniffi_loro_fn_free_localupdatecallback(pointer, status);
          return null;
        });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(status -> {
      ...
      return UniffiLib.INSTANCE.uniffi_loro_fn_clone_localupdatecallback(pointer, status);
    });
  }
    ...
    UniffiHelpers.uniffiRustCall( _status -> {
        UniffiLib.INSTANCE.uniffi_loro_fn_method_localupdatecallback_on_local_update(
            it, FfiConverterByteArray.INSTANCE.lower(update), _status);
    });
