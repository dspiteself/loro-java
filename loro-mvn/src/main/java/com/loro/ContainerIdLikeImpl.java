...
public class ContainerIdLikeImpl implements AutoCloseable, ContainerIdLike {
  ...
  public ContainerIdLikeImpl(Pointer pointer) {
   ...
   */
  public ContainerIdLikeImpl(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("ContainerIdLikeImpl object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("ContainerIdLikeImpl call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(status -> {
          UniffiLib.INSTANCE.uniffi_loro_fn_free_containeridlike(pointer, status);
          return null;
        });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(status -> {
      ...
      return UniffiLib.INSTANCE.uniffi_loro_fn_clone_containeridlike(pointer, status);
    });
  }
    ...
    UniffiHelpers.uniffiRustCall( _status -> {
        return UniffiLib.INSTANCE.uniffi_loro_fn_method_containeridlike_as_container_id(
            it, FfiConverterTypeContainerType.INSTANCE.lower(ty), _status);
    });
