...
public enum FfiConverterTypeOnPush implements FfiConverter<OnPush, Pointer> {
    INSTANCE;
    public final UniffiHandleMap<OnPush> handleMap = new UniffiHandleMap<>();

    @Override
    public Pointer lower(OnPush value) {
        return new Pointer(handleMap.insert(value));
    }

    @Override
    public OnPush lift(Pointer value) {
        return new OnPushImpl(value);
    }
